...
//$broker =json_decode($json);
?>
	<h1 class='article-page-head'><?php echo __("Agent Profile -",'wp2crm')." <span class='theme-color1'>". $broker->name;?></span></h1>
<div>
<?php the_content();?>
...
?>
		...
		<div class="property_detail"><label><i class="fa fa-user"></i>&nbsp;<? _e('Name','wp2crm');?>:</label> <?php echo $broker->name; ?></div>		
		<div class="property_detail"><label><i class="fa fa-certificate"></i>&nbsp;<? _e('Title','wp2crm');?>:</label> <?php echo $broker->c_title; ?></div>		
		<div class="property_detail"><label><i class="fa fa-mobile-phone"></i>&nbsp;<? _e('Mobile Phone','wp2crm');?>:</label> <?php echo $broker->c_mobile; ?></div>		
		<div class="property_detail"><label><i class="fa fa-phone"></i>&nbsp;<? _e('Work Phone','wp2crm');?>:</label> <?php echo $broker->c_office; ?></div>		
<?php
	if($broker->c_hiredate){
?> 
		<div class="property_detail"><label><i class="fa fa-thumbs-o-up"></i>&nbsp;<? _e('Active Since','wp2crm');?>:</label> <?php echo date("F Y",$broker->c_hiredate); ?></div>
<?php } ?>
...
<a name="list"></a><h1 class="article-page-head"><?php _e('My Business Listings','wp2crm'); ?> - <span class="theme-color1"><?php bloginfo('name');?></span></h1>
...
<i class="fa fa-phone"></i> <?php _e("Cell phone",'wp2crm');?>:<a href="tel:<?php echo $buyerbroker->c_mobile;?>"><?php echo $buyerbroker->c_mobile;?></a><br>
<i class="fa fa-phone"></i> <?php _e("Office phone",'wp2crm');?>:<a href="tel:<?php echo $buyerbroker->c_office;?>"><?php echo $buyerbroker->c_office;?></a><br>
<i class="fa fa-at"></i> <?php _e("Contact Agent",'wp2crm');?>:<a href="mailto:<?php echo $buyerbroker->c_email;?>"><?php echo $buyerbroker->c_email;?></a><br>
