...
?>
<section id="content" data="property" class="container"> 
...
if(count((array)$results) > 0 && $results->status != "404"){
	...
	if(!in_array($searchlisting->id,$listingids)){
		...
		$html .= "<div class='theme-color1-dk' style='display:inline-block; width:20%; font-size:1.3em;vertical-align:top;'>".__("","wp2crm").$searchlisting->c_listing_region_c;
	    
$html .= "<p style='font-size:1.3em;color:#4672b2;'>".__("",'wp2crm').$searchlisting->c_currency_id.number_format($searchlisting->c_listing_askingprice_c).'</p>';
	    $html .= "<p style='font-size:1.0em;color:#666666;'>".__("Cash Flow: ",'wp2crm').$searchlisting->c_currency_id.number_format($searchlisting->c_ownerscashflow)."</p>";
	$html .="</div>";
...
if(is_user_logged_in() ){
		$html .= '<form action="/listing/'.sanitize_title($searchlisting->c_name_generic_c).'" method=post><input type=hidden name="action" value="add_to_portfolio" /><input type=hidden name="id" value="'. $searchlisting->id.'" /><input type=submit style="display:none; margin-bottom:18px;" value="'. __('Add to my portfolio','wp2crm').' &#10010;" class="portfolio_action_button portfolio-add"  /></form>';
		}
		$html .= "</div>";
	}
	}
...
echo __("Your search for ",'wp2crm');
if(is_array($qy)){
	echo join(",",$qy);
}else {
	echo $qy;
...
echo (count((array)$listingids)===1)?__(' result.','wp2crm'):__(' results.','wp2crm');
}
