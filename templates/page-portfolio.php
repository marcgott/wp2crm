...
<h1 class="article-page-head"><?php _e('Data Room of', 'wp2crm');?> <?php echo $user->firstName.' '.$user->lastName ;?></h1>
...
echo '<div class="portfoliostatus added">&#10003; ' .	__("Your request for this address has been sent",'wp2crm') . "</div>";
//	echo '<input type=button onclick=location.href=\'/data-room\' class="portfolio_action_button" value="'. __("View Data Room",'wp2crm').'">';
...
?>
		<div class="property_detail"><label><?php _e("Listing ID:", 'wp2crm');?></label> <?php echo $listingitem->id; ?></div>
		<div class="property_detail"><label><?php _e("Listed on:", 'wp2crm');?></label> <?php echo date("F j, Y",$listingitem->c_listing_date_approved_c); ?></div>
		<div class="property_detail"><label><?php _e("Region:", 'wp2crm');?></label><?php echo $listingitem ->c_listing_region_c ;?></div>
		<div class="property_detail"><label><?php _e("Financial Info:", 'wp2crm');?></label></div>

		<div class="property_detail"><label><?php _e("Gross Sales:", 'wp2crm');?></label> <?php echo $listingitem ->c_currency_id;?><?php echo number_format($listingitem ->c_financial_sales_c);?></div>
		<div class="property_detail"><label><?php _e("Gross Revenue:", 'wp2crm');?></label> <?php echo $listingitem ->c_currency_id;?><?php echo number_format($listingitem ->c_financial_grossrevenue_c);?></div>
		<div class="property_detail"><label><?php _e("Owner's Cash Flow:", 'wp2crm');?></label> <?php echo $listingitem ->c_currency_id;?><?php echo number_format($listingitem ->c_financial_owner_cashflow_c);?></div>
		<div class="property_detail"><label><?php _e("Asking Price:", 'wp2crm');?></label> <?php echo $listingitem ->c_currency_id;?><?php echo number_format($listingitem ->c_listing_askingprice_c);?></div>
		<div class="property_detail"><label><?php _e("Down Payment:", 'wp2crm');?></label> <?php echo $listingitem ->c_currency_id;?><?php echo number_format($listingitem ->c_listing_downpayment_c);?></div>
<?php if($isaddressreleased):?>
	<div class="portfolio-released-address" style="border:1px solid #990000;padding:5px;">
		<h3><?php _e("Address", 'wp2crm');?></h3>
		<?php
		echo __("Business Name: ","wp2crm").$listingitem->c_name_dba_c.'<br>';
		echo __("Address: ","wp2crm").$listingitem->c_listing_address_c.'<br>';
		echo  $listingitem->c_listing_city_c." ".$listingitem->c_listing_region_c." ".$listingitem->c_listing_postal_c.'<br>';
		echo __("This address was released to you on ","wp2crm").date("F j, Y",$portfolioitem->c_date_released).'<br>';
		echo '</div>';
		endif; ?>

<br />
<a href="<?php echo $permalink;?>" class="portfoliobutton" data-id="<?php echo $listingitem->id;?>" class="portfoliobutton"><?php _e('View Listing Details','wp2crm');?></a>
