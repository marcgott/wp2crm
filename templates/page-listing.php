...
get_header();
?>
<link rel="stylesheet" href="/wp-content/plugins/wp2crm/css/bootstrap.min.css" type="text/css" media="all">
<style type="text/css">
.sidebar-primary {
		...
		<?php 
			if( is_user_logged_in() ){
				if($portfoliolisting->c_release_status== "Deleted"){
					echo '<div class="portfoliostatus deleted">&#10006; ' .	__("This property was removed from your data room",'wp2crm') . "</div>";
				}elseif($isaddressreleased){
					echo '<div class="portfoliostatus released"> &#9733; ' .	__("The address of this business is available to you",'wp2crm') . "</div>";
				}elseif($inportfolio){
					echo '<div class="portfoliostatus added">&#10003; ' .	__("This property is in your data room",'wp2crm') . "</div>";
				}
			}
		?>
		<div style="width:100% !important" id="divB" role="main">
			...
			<div id="divD" class="entry-content col-md-9" style="width:70%;display:inline-block; padding-right:10px;">
				...
				?>
						...
						<div id="divG" class="row"><!--this div is optional (extra info table)-->
							<div id="divH" class="col-md-4" style="border: 2px solid black;padding: 10px 5px;border-right: 0px;">
								<div style="width:100% !important; display:inline-block; float: left; border-bottom: 1px solid #6b9f67; text-align:left !important;margin-bottom: 5px;" class="">
									<label style="width: 100px !important;"><?php _e("Asking Price:", 'wp2crm');?></label> 
									<div style="background-color: #6b9f67; padding-left:7px;float:right; width:95px; "><?php echo $currency_symbol." ".$amount;?></div>
								</div> 
								<br>
								<div style="width:100% !important;display:inline-block; float: left; border-bottom: 1px solid #ddd; text-align:left !important;margin-bottom: 5px;" class=""><label ><?php _e("Gross Revenue:", 'wp2crm');?></label> <div style="background-color: #ddd; padding-left:7px;float:right; width:95px;  "><?php echo $currency_symbol." ".$grossrevenue;?></div></div><br>
								<div style="width:100% !important;display:inline-block; float: left; border-bottom: 1px solid #ddd; text-align:left !important;margin-bottom: 5px;" class=""><label style="width: 100px !important;"><?php _e("Cash Flow:", 'wp2crm');?></label> <div style="background-color: #ddd; padding-left:7px;float:right; width:95px; "><?php echo $currency_symbol." ".$ownercashflow;?></div ></div><br>
							</div><!--divH-->
					...
					<!--cont divD-->
					<h2 class="theme-color1"><?php _e("Business Description","wp2crm");?></h2>
					<div class=""><?php echo nl2br($description); ?></div>
					<hr>
					<?php
						$detailsheader = __("Detailed Information", 'wp2crm');
						if($isuserregistered && $inportfolio){
							$detailsheader = __("Complete Business Profile", 'wp2crm');
						}
					?>
					...
					<div id="propertydetails" class="property_details_div">
						<div id="divPD" class="property_details">
							<div class="property_detail"><label><?php _e("Listed on:", 'wp2crm');?></label> <?php echo date('F j, Y',$listing_dateapproved); ?></div>
							<div class="property_detail" id="property_listing_id" data-id="<?php echo $listing_id;?>"><label><?php _e("Listing ID:", 'wp2crm');?></label> #<?php echo $listing_id; ?></div>		
							<div class="property_detail"><label><?php _e("Categories:", 'wp2crm');?></label> <?php echo $categories; ?></div>
							<div class="property_detail"><label><?php _e("State:", 'wp2crm');?></label> <?php echo $region;?></div>
							<div class="property_detail"><label><?php _e("Down Payment:", 'wp2crm');?></label> <?php echo $currency_symbol." ".$downpayment;?></div>
							<div class="property_detail"><label><?php _e("Terms:", 'wp2crm');?></label> <?php echo $terms;?></div>
							<?php 
								if( $inportfolio ):
									//print_r($listing);
									if($isaddressreleased){
							?>
							<br />
							<h4 class=detailheader><?php _e("Location", 'wp2crm');?></h4>
							<div class="property_detail"><label><?php _e("Address:", 'wp2crm');?></label> <?php echo $listing->c_listing_address_c;?></div>
							<div class="property_detail"><label><?php _e("City:", 'wp2crm');?></label> <?php echo $listing->c_listing_city_c;?></div>
							<div class="property_detail"><label><?php _e("State:", 'wp2crm');?></label> <?php echo $listing->c_listing_region_c;?></div>					
							<div class="property_detail"><label><?php _e("Zip/Postal:", 'wp2crm');?></label> <?php echo $listing->c_listing_postal_c;?></div>
...
<!-- Gail's added tables start here -->	
				...
				<?php 
					elseif($isuserregistered):
						_e('For more details on this listing, please add it to your data room.','wp2crm');
					else:
						_e("In order to see more details about this listing, please contact your broker to become registered.",'wp2crm');
					endif; 
				?>
				</div><!--divPD-->
			</div><!--propertydetails-->
			...
			<?php
				if($portfoliolisting->c_release_status!= ""){
					echo '<input type=button class="theme-button1-lt" onclick=location.href=\''. get_permalink($wp2crm_option['wp2crm_loginbar_dataroom']). '\' class="portfolio_action_button portfolio-view" value="'.__('View Data Room','wp2crm').' &#10151;">';
				}
				if($inportfolio ){
			?>
					<input type=button class="theme-button1-lt" onclick=location.href=\'<?php echo get_permalink($wp2crm_option['wp2crm_loginbar_dataroom']);?>'  class="portfolio_action_button portfolio-remove"  value="<?php _e('Hide from Data Room','wp2crm');?> &#10006;">
			<?php
				}
				if($inportfolio && !$isaddressreleased && $portfoliolisting->c_release_status != "Requested"){
			?>
					<form method="post" style="display:inline"><input type=hidden name="uid" value="<?php echo $listing_id;?>"><input type=hidden name="action" value="request"><input type=submit class="theme-button1-lt" value="<?php _e('Request This Address','wp2crm');?> &#9733;"  class="portfolio_action_button request-address" ></form>
			<?php
				}
				if(is_user_logged_in() && !$inportfolio){
			?>
					<form method=post>
							<input type=submit class="theme-button1-lt" style="display:inline" value="<?php _e('Add to my data room','wp2crm');?> &#10010;" class="portfolio_action_button portfolio-add"  />
			...
			<div id="sidebar" class="col-md-3 sidebar" style="vertical-align:top;display:inline-block; ">
				<div id="inner-blue" class="theme-background1-dk" style="margin-bottom: 20px; color:#fff; width:100%;padding:18px; ">
					<div id="brokerdetails" class="broker_details_div">
						...
						<form method=POST action="<?php echo get_permalink($wp2crm_option["wp2crm_pageselect_broker"]);?>" id="brokerinfo">
