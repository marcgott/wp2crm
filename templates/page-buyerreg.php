...
#Create new CRM record
	...
	if(isset($_POST['listingid']) && $_POST["listingid"]!=""){
		...
		wp_redirect(get_permalink($wp2crm_option['wp2crm_pages_welcome']));
		exit;
	}else{
		echo $result[0];
	}
...
jQuery("#broker").change(function(){
	jQuery.getJSON('<?php echo plugins_url().'/wp2crm/_auth.php'; ?>',
...
jQuery("#email").blur(function(){
	...
	jQuery.get('<?php echo plugins_url().'/wp2crm/_auth.php'; ?>',{query:'AJAX',action: 'isvalidemail',email:emailaddr}, 
	function(response){
		if(!response){
			jQuery("#emailerr").remove();
		...
		//return false;		
			...
			jQuery.getJSON('<?php echo plugins_url().'/wp2crm/_auth.php'; ?>',{query:'AJAX',action: 'x2apicall',_class:'Contacts/by:email='+emailaddr+'.json'}, 
				function(response){
					if(!response.status){
						jQuery("#emailerr").remove();
...
//console.log(jQuery("#form_buyerreg").serialize())        	
			response = '';
				...
				jQuery.getJSON(
					'<?php echo plugins_url().'/wp2crm/_auth.php'; ?>',
					{
						query:	'AJAX',
						action: 'buyerreg_ajax_getnda',
						ndadoc: <?php echo $wp2crm_option["wp2crm_crm_nda"]?>,
...
<form id="form_buyerreg" action="<?php echo $wp2crm_option["wp2crm_pages_registration"];?>" method="post">
...
foreach($crmfields AS $idx=>$obj){
	if(in_array($obj->id,$wp2crm_option["wp2crm_buyerreg_fields"])){
...
?>
<?php
//print_r($wp2crm_option);
if($wp2crm_option["wp2crm_broker_showhide"]){
?>
...
?>
<input type=hidden name="assignedTo" value="<?php echo $wp2crm_option["wp2crm_crm_assignedTo"];?>" />
<?php
}
if($wp2crm_option["wp2crm_listing_showhide"]){
?>
