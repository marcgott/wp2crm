...
function wp2crm_add_wp2crm_rewrite() {
	global $wp_rewrite,$wp_query;	
		//flush_rewrite_rules();		
		add_rewrite_rule('^wp2crm\/newpass/(.+)\/(.+)\/?','index.php?newpass={"useremail":"$matches[1]","newpass":"$matches[2]"}','top'); 
		...
		add_rewrite_rule('^wp2crm/([A-Za-z]+)\/([0-9]+)\/Docs\/([0-9]+)\/?','index.php?wp2crm={"model":"$matches[1]","id":"$matches[2]","docId":"$matches[3]"}','top');
		
		add_rewrite_endpoint( 'wp2crm', EP_NONE );
		add_rewrite_endpoint( 'newpass', EP_ALL );
  
}
add_action( 'init', 'wp2crm_add_wp2crm_rewrite', 2,0 );

function wp2crm_newpass_api() {
	    global $wp_query;
...
function wp2crm_email_api() {
	global $wp_query;
	$showvars = '';
	if ( !isset( $wp_query->query_vars['wp2crm'] ) )
		return;
 
//echo "Need some anti-snooping security here";
	$wp2crm = json_decode(stripslashes($wp_query->query_vars['wp2crm']));

	$wp_query->query_vars['model']=$wp2crm->model;
	$wp_query->query_vars['id']=$wp2crm->id;
	$wp_query->query_vars['docId']=$wp2crm->docId;
...
add_action( 'template_redirect', 'wp2crm_email_api' );
add_action( 'template_redirect', 'wp2crm_newpass_api' );
